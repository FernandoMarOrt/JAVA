...
public class modificarCliente extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(140, 20, 340, 40));
        ...
        getContentPane().add(jTextField9, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 280, 240, 20));

        jLabel20.setForeground(new java.awt.Color(153, 102, 255));
        jLabel20.setText("ID Tarjeta");
        ...
        getContentPane().add(jLabel20, new org.netbeans.lib.awtextra.AbsoluteConstraints(130, 280, 90, -1));
    ...
    private void jButton7MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton7MouseClicked
        ...
        List<Cliente> listaClientes = cc.findClienteEntities(); //Relleno la lista con las facturas de la base de datos
        List<TarjetaDescuento> listaTarjetas = tc.findTarjetaDescuentoEntities();

        int idBuscada = Integer.parseInt(jTextField5.getText());

        Cliente cliente = cc.findCliente(idBuscada);

        if (listaClientes.contains(cliente)) { //Si la lista con tiene la factura con la pk proporcionada

            String dni = jTextField8.getText();

            //Si el dni coincide con alguno de los clientes le avisa
            if (listaClientes.stream().anyMatch(c -> c.getDni().equals(dni))) {

                JOptionPane.showMessageDialog(null, "El dni ya existe en la base de datos por favor introduce otro");
            } else {
                ...
                if (jTextField9.getText().isEmpty()) {
                    ...
                    try {
                        Utilidades.CreacionModificacionBorradoEntidades.modificarClienteSinTarjeta(cliente, nombre, apellidos, dni);
                    } catch (Exception ex) {
                    ...
                    int tarjetaID = Integer.parseInt(jTextField9.getText());

                    if (listaTarjetas.contains(tc.findTarjetaDescuento(tarjetaID))) { //Si la lista de tarjetas contiene la tarjeta que se quiere asociar al cliente

                        String nombre = jTextField6.getText();
                        String apellidos = jTextField7.getText();

                        try {
                            Utilidades.CreacionModificacionBorradoEntidades.modificarCliente(cliente, nombre, apellidos, dni, tc.findTarjetaDescuento(tarjetaID));
                        } catch (Exception ex) {
                            Logger.getLogger(modificarCliente.class.getName()).log(Level.SEVERE, null, ex);
                        }
                        ...
                        JOptionPane.showMessageDialog(null, "La tarjeta descuento que esta intentando asociar no existe");
                    }
